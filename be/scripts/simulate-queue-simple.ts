...
async function simulateWorkflow(queueId, customerCount = 10) {
  ...
  const customerNames = [
    ...
    "Agus Setiawan", "Rina Melati", "Dedi Kurniawan", "Maya Putri",
    "Indra Pratama", "Sarah Amalia", "Eko Prasetyo", "Fani Rahmawati",
    "Gunawan Wijaya", "Hartono Susilo", "Lestari Ayu", "Rizky Firmansyah",
  ];
  ...
  for (let i = 0; i < customerCount; i++) {
    ...
    if (!result.ok || !result.data.success) {
      console.error(`   ❌ Failed to create ${queueNumber}:`, result.data.error || result.data.message);
      continue;
    }
    
    const item = result.data.data;
    addedItems.push(item);
    console.log(`   ✅ [${Math.round(((i + 1) / customerCount) * 100)}%] ${queueNumber} - ${customerName} [${firstStatusLabel}]`);
...
async function runWorkflow(args) {
  ...
  if (!queueId) {
    ...
    console.log("Example: pnpm run simulate-simple workflow 019c6bd5-cbf2-7471-a6d8-bae2c09fd616 10");
    process.exit(1);
  }

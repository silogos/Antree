...
const config: SimulationConfig = {
  ...
  randomSeed: process.env.RANDOM_SEED ? parseInt(process.env.RANDOM_SEED) : undefined,
};
...
async function dryRunFetch<T = unknown>(
  ...
  console.log(`🔍 [DRY RUN] ${options.method || "GET"} ${config.apiBaseUrl}${endpoint}`);
...
export async function getBoardsByList(): Promise<SimulationResult<Board[]>> {
  try {
    console.log(`📋 Mengambil daftar queue dari ${config.apiBaseUrl}/queues...\n`);
    ...
    if (!result.ok || !result.data.success) {
      return {
        success: false,
        error: result.data.error || result.data.message || "Gagal mengambil queue",
        data: [],
      };
    }
    ...
    const boards = await Promise.all(
      result.data.data.map(async (queue: Queue): Promise<Board> => {
        ...
        if (!batchResult.ok || !batchResult.data.success || !batchResult.data.data) {
          return {
            queueId: queue.id,
...
export async function simulateBankQueue(
  ...
  try {
    ...
    for (let i = 1; i <= customerCount; i++) {
      ...
      const createResult = await fetchFn<{ success: boolean; data: QueueItem }>("/queue-items", {
        method: "POST",
...
export async function simulateStatusProgression(
  ...
  try {
    ...
    const fromStatus = statuses.find((s: Status) => s.label === fromStatusLabel);
    ...
    const itemsCount = itemCount ? Math.min(itemCount, itemsToMove.length) : itemsToMove.length;
    const itemsToProcess = itemsToMove.slice(0, itemsCount);

    console.log(`Ditemukan ${itemsToMove.length} item di status "${fromStatusLabel}".`);
    console.log(`Memindahkan ${itemsCount} item ke status "${toStatusLabel}"...\n`);
...
> {
  try {
    ...
    console.log(`   Delay Progres Melayani: ${formatDuration(serveProgressDelay)}`);
    console.log(`   Pola Delay: ${delayPattern}\n`);
...
const commands: CLICommand[] = [
    ...
    handler: async () => {
      ...
      result.data.forEach((board: any, index: number) => {
        ...
        if (board.hasActiveBatch) {
          console.log(`   Batch Aktif: ${board.batchName} (${board.activeBatchId})`);
          console.log(`   Status: ${board.statuses.map((s: any) => s.label).join(" → ")}`);
        } else {
          console.log("   ⚠️  Tidak ada batch aktif - silakan buat batch aktif terlebih dah");
        }
        console.log("");
      });
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const delayPattern = (args[3] as "fixed" | "random" | "poisson" | "weighted") || "fixed";
      ...
      if (!boardId) {
        ...
        console.log("Contoh: npm run simulate simulate queue-id-123 2000 10 random 50");
        process.exit(1);
      }
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const delayPattern = (args[5] as "fixed" | "random" | "weighted") || "fixed";
      const variancePercent = parseInt(args[6]) || 30;

      if (!boardId || !fromStatusLabel || !toStatusLabel) {
        console.error("❌ Error: boardId, fromStatusLabel, dan toStatusLabel diperlukan");
        console.log(
  ...
  {
    name: "workflow",
    description: "Simulasi lengkap: tambah customer dan pindahkan melalui semua statuses",
    handler: async () => {
      const args = process.argv.slice(3);
      const boardId = args[0];
      ...
      const delayPattern = (args[5] as "fixed" | "random" | "poisson" | "weighted") || "fixed";
      const variancePercent = parseInt(args[6]) || 30;
      ...
      if (!boardId) {
        ...
        console.log("Contoh: npm run simulate workflow queue-id-123 5 1500 3000 2000 random 30");
        process.exit(1);
      }

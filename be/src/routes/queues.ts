...
queueRoutes.get("/:id/active-batch", async (c) => {
	try {
		...
		if (!result) {
			return c.json(
				successResponse(null, "No active batch found for this queue"),
			);
		}
...
queueRoutes.post("/", validateBody(createQueueSchema), async (c) => {
	try {
		...
		if (!queue) {
			return c.json(successResponse(null, "Template not found"), 404);
		}
...
queueRoutes.post("/:id/reset", validateBody(resetQueueSchema), async (c) => {
	try {
		const id = c.req.param("id");
		const input = c.get("validatedBody") as Parameters<
			typeof queueService.resetQueue
		>[1];
		...
		return c.json(
			successResponse(batch, "Queue reset successfully - new batch created"),
		);
	} catch (error) {
...
/**
 * GET /queues/:id/items
 * Get all queue items for a specific queue (from active batch)
 * Optionally filter by status using query parameter ?statusId=xxx
 */
queueRoutes.get("/:id/items", async (c) => {
	try {
		const id = c.req.param("id");
		...
		if (!queue?.activeBatchId)
			return c.json(internalErrorResponse(new Error("No Actived batch")), 404);

		const items = await queueService.getQueueItems(queue.activeBatchId);

		return c.json(successResponse(items, undefined, items.length));
	} catch (error) {
		console.error("[QueueRoutes] GET /queues/:id/items error:", error);
		return c.json(internalErrorResponse(error), 500);
	}
});

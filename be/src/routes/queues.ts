...
queuesRoutes.get('/:queueId/sessions', async (c) => {
  try {
    ...
    const status = c.req.query('status') as 'active' | 'paused' | 'completed' | 'archived' | undefined;
 ...
 */
queuesRoutes.post('/:queueId/sessions', validateBody(createSessionSchema), async (c) => {
  try {
    const queueId = c.req.param('queueId');

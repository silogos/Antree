...
templateRoutes.post("/", validateBody(createTemplateSchema), async (c) => {
	try {
		const input = c.get("validatedBody") as Parameters<
			typeof templateService.createTemplate
		>[0];
...
templateRoutes.post(
	"/:id/statuses",
	validateBody(createTemplateStatusSchema),
	async (c) => {
		try {
			const id = c.req.param("id");
			const input = c.get("validatedBody") as Parameters<
				typeof templateService.addTemplateStatus
			>[1];
			...
			return c.json(
				successResponse(status, "Template status created successfully"),
				201,
			);
		} catch (error) {
			console.error(
				"[TemplateRoutes] POST /templates/:id/statuses error:",
				error,
			);
			return c.json(internalErrorResponse(error), 500);
		}

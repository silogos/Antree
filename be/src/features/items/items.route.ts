...
import { createQueueItemViaSessionSchema, updateQueueItemSchema } from "./queue-item.validator.js";
...
sessionItemRoutes.post(
  ...
  async (c) => {
    try {
      ...
      console.error("[ItemRoutes] POST /sessions/:sessionId/items error:", error);
      return c.json(internalErrorResponse(error), 500);
    }
  }
);

...
sessionRoutes.patch("/:id", validateBody(updateSessionSchema), async (c) => {
  try {
    const id = c.req.param("id");
    const input = c.get("validatedBody") as Parameters<typeof sessionService.updateSession>[1];
 ...
 */
sessionRoutes.patch("/:id/lifecycle", validateBody(lifecycleUpdateSchema), async (c) => {
  try {
    const id = c.req.param("id");
    const input = c.get("validatedBody") as Parameters<typeof sessionService.updateSession>[1];
    const session = await sessionService.updateSession(id, input);
    ...
    return c.json(successResponse(session, "Session lifecycle updated successfully"));
  } catch (error) {
...
sessionRoutes.patch("/session-statuses/:id", async (c) => {
  try {
    ...
    return c.json(successResponse(updated, "Session status updated successfully"));
  } catch (error) {

...
import type { CreateQueueInput, ResetQueueInput, UpdateQueueInput } from "./queue.validator.js";
...
export class QueueService {
  ...
  async getAllQueues(pagination?: PaginationParams): Promise<PaginatedResponse<Queue>> {
    ...
    // Get total count
    const [{ count }] = await db.select({ count: sql<number>`count(*)::int` }).from(queues);
  ...
  async getQueueItems(
    ...
    const conditions: ReturnType<typeof eq>[] = [eq(queueItems.sessionId, sessionId)];
  ...
  async getActiveSession(queueId: string): Promise<{
    ...
    const activeSession = await db
      .select()
      .from(queueSessions)
      .where(and(eq(queueSessions.queueId, queueId), eq(queueSessions.status, "active")))
      .limit(1);
  ...
  async updateQueue(id: string, input: UpdateQueueInput): Promise<Queue | null> {
    ...
    const result = await db.update(queues).set(updateData).where(eq(queues.id, id)).returning();
   ...
   */
  async resetQueue(id: string, input: ResetQueueInput = {}): Promise<QueueSession | null> {
    // Verify queue exists
    const queue = await db.select().from(queues).where(eq(queues.id, id)).limit(1);
    if (!queue || queue.length === 0) {
      return null;
    ...
    const currentActiveSession = await db
      .select()
      .from(queueSessions)
      .where(and(eq(queueSessions.queueId, id), eq(queueSessions.status, "active")))
      .limit(1);

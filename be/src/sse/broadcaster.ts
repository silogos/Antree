...
class SSEBroadcaster {
  ...
  private clientMessageCounts: Map<string, { count: number; resetTime: number }> = new Map();
  ...
  addConnection(
    ...
    if (existingConnections && existingConnections.length >= this.connectionLimit) {
      logger.warn({ sessionId, limit: this.connectionLimit }, "[SSE] Connection limit reached");
      return false;
    }
    ...
    logger.info({ clientId, sessionId, lastEventId }, "[SSE] Client connected");
  ...
  removeConnection(sessionId: string, clientId: string): void {
    ...
    if (boardConnections) {
      ...
      if (index !== -1) {
        ...
        logger.info({ clientId, sessionId }, "[SSE] Client disconnected");
      }
    ...
    this.connections.forEach((conns, _sessionId) => {
      conns.forEach((conn) => {
        try {
          ...
          logger.error({ clientId: conn.clientId, error }, "[SSE] Error closing connection");
        }
      });
    });

...
const errorMessages: Record<
  ErrorCode,
  { title: string; message: string; severity: ErrorSeverity; suggestions?: string[] }
> = {
  // Validation errors
  ...
  [ErrorCode.FOREIGN_KEY_VIOLATION]: {
    ...
    severity: ErrorSeverity.Warning,
    suggestions: [
      "Remove or reassign any dependent records first",
      "You may need to delete related items first",
    ],
  },
  [ErrorCode.IN_USE]: {
    title: "Item In Use",
    ...
    suggestions: [
      ...
      "Close any open sessions using this item",
    ],
  },
...
export function formatErrorForDisplay(error: unknown): {
  ...
  return {
    ...
    showSupportContact:
      parsed.severity === ErrorSeverity.Error || parsed.severity === ErrorSeverity.Critical,
  };
}

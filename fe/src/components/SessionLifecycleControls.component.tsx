...
export function SessionLifecycleControls({
  ...
  const handleLifecycleChange = async (newLifecycle: SessionLifecycle) => {
    try {
      const updatedSession = await sessionService.updateSessionLifecycle(id, newLifecycle);
    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`px-3 py-1 rounded-full text-xs font-medium text-white ${getBadgeColor(currentStatus)}`}
          >

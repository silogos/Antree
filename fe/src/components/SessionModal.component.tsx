...
import { CheckCircle2, LayoutGrid, Trash2 } from "lucide-react";
...
export function SessionModal({
        ...
        if (response.success && response.data) {
          ...
          const templatesWithStatuses = await Promise.all(
            response.data.map(async (template) => {
              try {
                const statusesResponse = await http.get<{
                  success: boolean;
                  data?: QueueTemplateStatus[];
          ...
          const defaultTemplate = sortedTemplates.find((t) => t.id === queueTemplateId);
          if (defaultTemplate) {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      onSuccess?.();
      onClose();
      success(isEditing ? "Session updated successfully!" : "Session created successfully!", {
        description: values.name,
      });
    } catch (err) {
      console.error("Failed to save session:", err);
      showError(isEditing ? "Failed to update session" : "Failed to create session", {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showError("Failed to delete session", { description: "Please try again later." });
    } finally {
      setIsDeleting(false);
    }
  };

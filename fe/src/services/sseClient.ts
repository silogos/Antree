...
export class SSEClient {
	...
	private scheduleReconnect(isSession: boolean, id: string): void {
		...
		this.reconnectTimeout = setTimeout(() => {
			if (!this.isManualDisconnect && this.url) {
				console.log(
					`[SSE Client] Attempting reconnection #${this.reconnectAttempts}`,
				);
	...
	connect(batchId: string, options: SSEClientOptions = {}): void {
		...
		if (options.maxReconnectDelay) {
			(this as any).maxReconnectDelay = options.maxReconnectDelay;
		}
		if (options.initialReconnectDelay) {
			(this as any).initialReconnectDelay = options.initialReconnectDelay;
		}
